  ...
  return (
    <div className="w-80 rounded-2xl bg-white shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden group border">
      ...
      <div className="p-5 space-y-3">
        ...
        {/* Button */}
          {
          isInCart ? (<button className="text-black w-full bg-gray-600 flex items-center justify-center gap-4">
          <span onClick={()=>decrementQuantity(product.id)} className="text-4xl">-</span>
          <span className="text-4xl">{isInCart.quantity}</span>
          <span onClick={()=>incrementQuantity(product.id)} className="text-4xl">+</span></button>)
          :
