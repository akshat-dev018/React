  ...
  return (
    <div className='p-4 border border-white rounded flex flex-col gap-2 bg-black text-white '>
        ...
        <div className='flex justify-between w-full gap-4'>
            <button onClick={()=>{
                setUpdatedData(user)
                setToggle(prev=>!prev)}} className='bg-yellow-700 text-white py-2 px-3 rounded'>Update</button>
            <button onClick={()=>deleteUser(user.id)} className='bg-red-700 text-white py-2 px-3 rounded'>Delete</button>
        </div>
    </div>
  )
}
